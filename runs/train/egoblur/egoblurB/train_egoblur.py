...
TRAIN_SET, EVAL_SET = "train_Z3", "faceback_hq_v2_eval"   # TRAIN_SET is overridden by cfg.DATASETS.TRAIN[0] in main()
...
def main(args):
    global ARGS, TRAIN_SET
    ARGS = args
    cfg = setup(args)
    TRAIN_SET = cfg.DATASETS.TRAIN[0]   # egoblurA: train_Z3 (139,587); egoblurB: train_Z3_full (223,214), PII-1065
    register(TRAIN_SET, str(D2_DIR / f"{TRAIN_SET}.json"))
    register(EVAL_SET, str(D2_DIR / f"{EVAL_SET}.json"))
    tsum = json.load(open(D2_DIR / f"{TRAIN_SET}.summary.json"))
    log.info(f"train set {TRAIN_SET}: {tsum['n_images']} images, {tsum['boxes']} boxes, {tsum['ignore']} ignore "
             f"(json md5 {tsum['json_md5']})")
