...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  wait_for_gpus
  RESUME=""
  # WOR-196 EMA-aware resume (armAG34 copy, WOR-1174): tools/train.py --resume-from loads the checkpoint before EMAHook
  # registers its ema_* buffers, so also pass custom_hooks.0.resume_from (consumed by the --cfg-options list above).
