"""PII-1440: copy of the PII-1248 armAG34 exporter (itself the WOR-206 armAE34 exporter, md5 d6704cf1) for armAM34: only CKPT/OUT and
the print labels change; the 443 ema_* EMA audit, CFG, opset and graph contract are unchanged.
Original docstring: copy of export_armAA34_onnx.py (itself the WOR-146 copy of export_armY_onnx.py) for
armAE34: CKPT/OUT changed and the strict key audit replaced by the WOR-201 EMA variant.
armAE34 inherits scrfd_armAA34.py, so CFG stays the armY embedded 34G config; armAM34 (fluence job 12804) is the same 34G architecture.
WOR-204: the checkpoint carries one ema_<param> buffer per PARAMETER and 34G has 443 parameter
tensors, so the audit must see exactly 443 ema_* unexpected keys and 0 missing. The parameter
slots already hold the EMA weights (EMAHook priority 49 runs before CheckpointHook), no swap.
...
CKPT = Path("/data/esteban/pii/runs/train/wd_armAM34_fluence/epoch_20.pth")
OUT = Path("/data/esteban/pii/runs/train/armAM34_det34g.onnx")
...
def main() -> None:
    ...
    # WOR-204/WOR-201: armAE34 trained with mmcv EMAHook; epoch_20.pth carries one ema_<param>
    # buffer per parameter (443 for 34G) next to the model keys, and the parameter slots already
    # hold the EMA weights (hook order EMAHook 49 before CheckpointHook 50), so no swap: the audit
    # expects exactly those 443 ema_* keys as unexpected, nothing else, and 0 missing.
    ema = [k for k in unexpected if k.startswith("ema_")]
    params = {n.replace(".", "_"): n for n, _ in model.named_parameters()}
    bad_ema = [k for k in ema if k[4:] not in params]
    print(f"key audit: missing={missing} unexpected={len(unexpected)} (ema_*: {len(ema)}, "
          f"non-ema: {[k for k in unexpected if k not in ema]}, "
          f"ema not naming a param: {bad_ema}, n_params: {len(params)})", flush=True)
    assert not missing, f"missing keys: {missing}"
    assert len(ema) == 443 and len(unexpected) == 443 and not bad_ema and len(params) == 443, \
        "armAM34 EMA audit failed (expected missing=[] and 443 ema_* unexpected keys)"
    ...
    print("\n=== graph contract: armAM34 vs production det_10g.onnx ===")
    a, b = describe(OUT), describe(PROD_ONNX)
    for k in ("size_mb", "opset", "ir", "n_nodes"):
        same = a[k] == b[k]
        print(f"  {k:9} ours={a[k]}   prod={b[k]}{'' if same else '   <-- differs'}")
    print(f"  inputs    ours={a['inputs']}")
    print(f"            prod={b['inputs']}"
          f"{'' if a['inputs'] == b['inputs'] else '   <-- differs'}")
    print(f"  n outputs ours={len(a['outputs'])}  prod={len(b['outputs'])}")
