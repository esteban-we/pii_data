"""PII-1036: copy of export_armAA34_onnx.py (the armAE34 exporter's parent) for the UNTUNED official
SCRFD-34G checkpoint (CKPT/OUT changed; CFG stays the armY embedded 34G config). One departure: the
official checkpoint has no kps head (its own config says use_kps=False), so the strict key audit
allows exactly bbox_head.stride_kps.0.{weight,bias} to be missing and zeroes them after loading.
Everything else (venv, opset 11, 640x640 dummy, dynamic axes, 9 outputs, graph contract) is unchanged.

Original docstring: copy of export_armY_onnx.py for armAA34 (CKPT/OUT changed; CFG stays the armY embedded 34G config, whose model block is byte-identical to scrfd_armZ34.py).
...
CKPT = Path("/data/esteban/pii/weights/SCRFD_34G_official.pth")
OUT = Path("/data/esteban/pii/weights/stock34_det34g.onnx")
PROD_ONNX = Path("/data/esteban/pii/weights/det_10g.onnx")

KPS_KEYS = ["bbox_head.stride_kps.0.bias", "bbox_head.stride_kps.0.weight"]
...
def main() -> None:
    ...
    assert missing == KPS_KEYS and not unexpected, "architecture/checkpoint mismatch (only the kps head may be absent)"

    load_checkpoint(model, str(CKPT), map_location="cpu", strict=False)
    # PII-1036: the official 34G checkpoint was trained with use_kps=False and carries no kps head;
    # zero that leaf conv so kps_8/16/32 are exactly 0 (deterministic, no random init). Boxes and
    # scores never touch it (stride_kps reads reg_feat and feeds nothing else).
    with torch.no_grad():
        for k in KPS_KEYS:
            model.state_dict()[k].zero_()
    assert all(float(model.state_dict()[k].abs().sum()) == 0.0 for k in KPS_KEYS)
    print(f"kps head zeroed: {KPS_KEYS}", flush=True)
    model.cpu().eval()
