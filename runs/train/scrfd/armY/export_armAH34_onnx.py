"""PII-1288: copy of the PII-1248 armAG34 exporter (itself the WOR-206 armAE34 exporter, md5 d6704cf1) for armAH34: only CKPT/OUT and
...
armAE34 inherits scrfd_armAA34.py, so CFG stays the armY embedded 34G config; armAH34 (fluence job 8574) is the same 34G architecture.
...
CKPT = Path("/data/esteban/pii/runs/train/wd_armAH34_fluence/epoch_20.pth")
OUT = Path("/data/esteban/pii/runs/train/armAH34_det34g.onnx")
...
def main() -> None:
    ...
    assert len(ema) == 443 and len(unexpected) == 443 and not bad_ema and len(params) == 443, \
        "armAH34 EMA audit failed (expected missing=[] and 443 ema_* unexpected keys)"
    ...
    print("\n=== graph contract: armAH34 vs production det_10g.onnx ===")
