...
Round 3 of the faceight annotation program: 40,000 frames, both eyes (18,994 left,
21,006 right), all 2328x1748, from 18,704 episodes (one session and one chunk each). The
rows are `annotation_round == 3` in `data/faceight/frames.csv` of the fpv_face_pii repo.
...
from `oss://we-vlm-annotation-data-sh/faceight_c/images/` with GetObject (128 parallel
requests, 24,583,618,577 bytes in 319 s, 77.0 MB/s, 0 failures, 0 retries), each object
...
```
images/                 40,000 jpg, 18,994 left and 21,006 right, 24,583,618,577 bytes, all 2328x1748
...
- `oss_key`: the eye-less Verdict object
  `faceight_c/images/<session>_c<chunk>_f<frame_idx:06d>.jpg`. The vendor portal accepts
  no eye token in a file name, so the eye lives in the `view` field of every job file.
  Those names do not collide inside this round: the build asserts it, and the 40,000 OSS
  keys match the 40,000 `frames.csv` rows one to one in both directions.
...
`split.csv`: 15,008 train and 3,696 eval sessions, 32,120 train and 7,880 eval frames.
...
`data/splits/faceight_c_{train,eval}_episodes_v1.txt`). 17,266 of the 18,704 round-3
episodes also carry frames of another round (11,912 of round 1, 15,069 of round 2); an
...
- images: `oss://we-vlm-annotation-data-sh/faceight_c/images/` (the package pushed for
...
- `split.csv`: `data/splits/faceight_c_{train,eval}_episodes_v1.txt`.
- boxes and job files: see the per-version READMEs.
- The old `/data/esteban/pii/datasets/faceight_c` held no images, only `boxes.jsonl`, the
  labels drop, a README and two symlinks into the shang frame trees.
