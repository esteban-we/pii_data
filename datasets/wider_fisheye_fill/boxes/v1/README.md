# wider_fisheye_fill boxes v1: the WIDER boxes mapped through the fisheye warp

## Why

The only label pass this set has: the generator (`training/warp_wider_fisheye.py --version
2`) forward-maps every WIDER FACE training box through the same virtual camera it used to
...
`boxes.csv`: 152,235 rows on 12,810 images (95.5% of the 159,390 source boxes; 6,812
dropped as not visible, 343 as too small, per `derivation/wider_fisheye_stats.json`);
69 images have no row (4 of them had no box in WIDER either). `frames.csv`:
`reviewed=1` on all 12,879 images.

`build_wider_pii2.py --set wider_fisheye_fill` converts the labelv2 file (19-field lines: four
